...
class DashboardController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if (!Yii::$app->user->can('admin')) {
            ...
            $usersQuery->andFilterWhere(['app_id' => count($formIds) > 0 ? $formIds : 0]);
            $totalUsersQuery->andFilterWhere(['app_id' => count($formIds) > 0 ? $formIds : 0]);
            $submissionsQuery->andFilterWhere(['form_id' => count($formIds) > 0 ? $formIds : 0]);
            $totalSubmissionsQuery->andFilterWhere(['form_id' => count($formIds) > 0 ? $formIds : 0]);
        }
        ...
        if (!Yii::$app->user->can('admin')) {
            // Add user filter to queries
            $formsByUsersQuery->andFilterWhere(['f.id' => count($formIds) > 0 ? $formIds : 0]);
            $formsBySubmissionsQuery->andFilterWhere(['fs.form_id' => count($formIds) > 0 ? $formIds : 0]);
            $lastUpdatedFormsQuery->andFilterWhere(['f.id' => count($formIds) > 0 ? $formIds : 0]);
        }
